...
const solution = (times) => {
  ...
  for (let time of times) {
    for (let i = 0; i < etTable.length; i++) {
      ...
      if (etTable[i] === time[0]) {
        people--;
        console.log(etTable[i], "이미 나간 애 -1", people);
      } else {
        people++;
        newEtTable.push(etTable[i]);
        console.log(etTable[i], "아직 있는 애 +1", people);
      }
    }
    etTable = [...newEtTable, time[1]];
    newEtTable = [];
  }
...
let arr = [
  ...
  [5, 14],
  [14, 15],
]; // [[ 5, 14 ],[ 12, 15 ],[ 14, 15 ],[ 14, 18 ],[ 15, 20 ],[ 20, 30 ]]
